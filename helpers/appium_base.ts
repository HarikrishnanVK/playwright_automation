...
export class AppiumBase {
    ...
    public waitTime = async (seconds: number) => await new Promise(resolve => setTimeout(resolve, seconds * 1000))
    ...
    public async closeApp(): Promise<void> {
        try {
            logger.info("Stop the CMH application");
            await driver.closeApp()
        }
        catch (error) {
            logger.error(`Stop the dynamo application failed. Exception ${error}`);
        }
    }
