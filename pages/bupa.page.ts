...
export class BupaPage extends WebBase {
  ...
  async selectCountriesFromList(country: string) {
    await this.selectDataFromDropdown(this.countriesDropDown, country)
  ...
  async mouseHoverOnLink(linkName: string, isMobile: Boolean) {
    ...
    if (isMobile) {
      ...
      for (let i = 0; i < 5; i++) {
        let isDisplayed = await this.returnElementStatus("input[id=mobiSearchTextbox]")
        if (!isDisplayed) {
  ...
  async verifySearchResult(content: string) {
    const locator = await this.findWebElement(this.resultSummary)
    await expect(locator.nth(0)).toContainText(content, { timeout: 20000 })
    const results = await this.getAllText(this.resultSummary)
    expect(results.toString()).toContain(content)
    const resultLinks = await this.getAllText(this.resultLinks)
    expect(resultLinks.toString().toLowerCase()).toContain(content.toLowerCase())
  }

  async navigateToFirstLink() {
    const firstLinkResult = await (await this.findWebElement(this.resultLinks)).nth(1).textContent({ timeout: 60000 })
    ...
    await (await this.findWebElement(this.resultLinks)).nth(1).click()
  }
  ...
  private async switchToWindow(): Promise<Page | undefined> {
  ...
  async verifyLocation(countryLink: string) {
    let newPage = await this.switchToWindow();
  ...
  async clickMediaLinks(siteName: string, isMobile: Boolean) {
    let links: Locator
    if (isMobile) {
      links = await this.findWebElement(this.serviceLinksInMobile)
      ...
      links = await this.findWebElement(this.serviceLinks)
      for (let i = 0; i < await links.count(); i++) {
        const attributeValue: any = await links.nth(i).getAttribute('href')
        if (attributeValue.match(siteName)) {
          await links.nth(i).click()
